...
from signal_agent.content.wtpu_channel import run_wtpu_channel, WTPUOutput, WTPU_OUT_DIR

@pytest.fixture
def mock_agent():
    with patch("signal_agent.content.wtpu_channel.SignalAgent") as mock_class:
        agent_instance = mock_class.return_value
        
        # Simulated determinisitc response from LLM
        mock_response = '''```json
{
...
def test_wtpu_channel_isolation_and_completeness(mock_agent, tmp_path):
    ...
    with patch("signal_agent.content.wtpu_channel.WTPU_OUT_DIR", tmp_path / "wtpu"):
        ...
        output = run_wtpu_channel(thought)
