...
| `CLOSE-040` | Commit antiglue governance evidence | Governance evidence | `ready` | antiglue and governance evidence tests/files; deferred shared health/reaction files | Lifecycle/reconcile prerequisite is closed, but evidence files still need exact diff review before staging. | Classify `tests/test_antiglue_phase_next.py`, `tests/test_governance_unification.py`, `shared/health.py`, `shared/event_reader.py`, `shared/artifact_envelope.py`, and `shared/reactions.py` one by one. | Dependent evidence commit passes targeted verification without prerequisite mixing. |
...
## Deferred From Slice 003

These files were intentionally excluded from `CLOSE-030` and must be classified one by one before any later staging:
